...
在TB之外还有一个BB，Basic Block，的概念，它的定义是一个中间没有跳转的TB。这个说
法给了我很大的误导，因为照理说遇到跳转，本来就会产生新的TB，所以为什么又需要一
个BB的概念呢？看了TCGv的用法后，我猜是这样的：这里的“中间没有跳转”，指的是T上下
文没有跳转，不是G上下文没有跳转。如果你在TCG中用tcg_gen_br()一类的指令生成跳转，
跳转之内的指令才是一个BB，跳转过后就不是BB了。所以代码中并没有一个叫BB的实体，
BB是一个逻辑上的概念，它主要对TCGv的生命周期管理起作用。
...
1. 普通TCG变量：通过tcg_temp_new()等函数创建的临时变量，它们只在一个BB之内有效。
   ...
   在实现上，每次发生TCG的跳转（比如tcg_jump, tcg_br，tcg_brcond等），TCG会做一
   次资源回收，这个寄存器会被设置成TS_DEAD，在后续流程中进行资源回收。

2. 本地TCG变量：通过tcg_temp_local_new()创建，它在一个TB内有效，需要主动free。

   在实现上，如果你不主动free这个变量，这个变量会被留在TB中，如果TB没有被清除，
   下次执行这个TB的时候，这个变量还会在TB的上下文中。

3. 全局TCG变量：这种可以跨越BB和TB，一直有效。
...
1. tcg_gen_lookup_and_goto_ptr()，这个函数在TB里产生一个跳转代码，这个段跳转代
   ...
   率比退出TB高，到因为每次都要查找，而且用绝对跳转，这个效率也有点低。
...
2. tcg_gen_goto_tb()，这个函数在TB里面产生一个调用桩，第一次设置的时候，它跳到
   ...
   提供了两个目标地址供固定关联，用来处理if/else两个固定链接点。（这个方法在
   User模拟的时候还有更多限制，这里不深入到那个细节了）

TCG也提供一些gen函数用于辅助完成一些特殊代码的生成，比如：

1. TB间直接跳转（不经过），这通过设置PC寄存器，然后调用
   tcg_gen_lookup_and_goto_ptr()完成。加入这个翻译后，这个TB需要结束，这通过设
...
PageDesc中就记录这个TB的相关信息，之后如果这个PageDesc被修改了，对应的TB也会被
清除掉，这样才能保证代码正常。这也导致了，在跨Page的时候，不能产生goto_tb的代码，
这常常很影响性能。

.. note::

  对于用户态模拟，如果确定我们不会使用自修改代码，有时我会关掉goto_tb的这个生成
  代码，这样有时可以大大提高运行效率。但这样是有风险了，看你的需要了。
