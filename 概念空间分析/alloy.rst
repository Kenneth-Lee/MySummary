...
所以，我建议读者可以去翻一下CHARLES C. PINTER教授的《A Book of Set Theroy》，看
完前半段就够了，把相关词语的概念复习一下，然后后面再看后面的概念就容易多了。

我这里重点把其中的要点列出来，帮助已经有相关概念，但部分细节已经想不起来的读者
建立一个针对Alloy的，有结构的逻辑空间。
...
你必须从语义上理解它。为了更容易理解它，数学家们定义了谓词的概念。简单说，我不
管你那些什么“狗”啊，“卡拉”啊，这些概念，我们就谈某个集合，把对这个集合的属性进
行定义（范围定义）的描述都用一个函数来表示，这样，我们就不再说狗有四条腿了，“有
四条腿”这个谓语我们说f(x)，其中x是狗的集合，所以卡拉有四条腿就被定义成：“存在狗
x，使得f(x)”。这就更接近数学了。这叫谓词逻辑（Predicate Logic，或者
Quantification logic）。

.. note::

   从谓词逻辑理解维特根斯坦的理论会很简单，一旦你用上文这个方法来理解逻辑，你很
   自然就会理解什么是Can be said clearly和Passed Over In Silence了。存在x属于集
   合A，使得f(x)成立，这是逻辑，是Can be said clearly的。而A是狗的集合，这个事
   情每个人理解都不一样，这就只能“你知我知”，就是只能Passed Over In Silence了。

   这也是《道德经》中的名和道的区别，名就是你已经总结的逻辑元素，它来自集合的分割
   方法（把什么定义为“集合内”，有；把什么定义为“集合外”，无）。

基于谓词逻辑，我们对于每件事的叙述（比如“x有四条腿”），都是一种范围的收缩。都构
成一个集合。

让我提前用一些Alloy的例子来让你理解这一点：比如说，我们讨论水果的颜色，假定我们
都是小孩子，知道的东西也只有有限的水果和颜色。那么，在我们的世界中，我们只有三种
水果：苹果，橙子和梨子（请注意，我们并没有很多的苹果，很多的梨子，我们只有一个
苹果，一个梨子，一个橙子）。而颜色我们只认识两种：红，不红。在我们的世界中，我
们只有这五个实体，它们互相之间都可能有关系，也可能没有关系，假定我们的脑子更差
一点，只能建立二元的关系：比如我们认为苹果是红的。或者苹果是梨子的母亲，都行，
反正这五者两两之间才能发生关系，我们的脑子认识不了三个或者三个以上的关系。那么
在我们没有对这个世界建立任何其他认识之前，这个世界的可能关系的空间，就包含2的20
次方个组合（每个成员都有是否和另四个成员有没有关系的可能性，这需要4个bit表示，5
个成员总共20个bit）。用计算机的单位，这是1M的信息。
...
.. note::
   ...
   奇怪，但如果你建立了集合论的概念，你就会发现其实他说得很直白了：他说的World，
   并不是我们以为我们认识了的那个世界，而是真正的世界在我们头脑中的那个“数字孪
   生”，是抛弃了Pass Over In Silence而剩下的那个数字建模。缺乏哲学思维的人总是
   把两者等同，但其实你的数字孪生是对真实世界的一个抽象和建模，并不包含所有的信
   息。

   理解这一点，《道德经》里面的道和名也就很好理解了。道就是外部那个影响你的告知
...
在直觉上，我们可能认为“如果苹果是红的，那么梨子就不是不红的”不是一种范围收缩，
而是一种”推理“。但请用前面那个全集来想这个问题：这个推理本质不就是说：当苹果这
个对象和红这个对象发生了关联，那么梨子和不红那个对象就一定有关联吗？不就是不能
再认为我们的世界中不能存在苹果红，梨子也红这种组合了吗？
...
.. warning::
   ...
   界进行限制，得到一组集合A1，A2，A3，...，这组我们设计的An能够包住现实造成的
   限制Bn。我们希望达成的目的是：A1交A2交A3...是T的子集。
...
这种定义方法在变量前面加上了x的取值范围的约束符，被称为量词，所谓这种谓词逻辑，
又称为“量词逻辑”（Quantification Logic）。

贝尔悖论
--------

另一个经典悖论是“Barry悖论”，又称为语义悖论。它的例子描述是这样的：假定有一本固
定单词数量的英语书。我们定义集合Q是这本英语书的20个单词以内可以表达的所有自然数。
由于英语书的单词有限，我们可以认为Q是一个有限的集合。那么我们可以认为Q有一个最
大值。那么，我们可以用英语说：“比Q的最大值大一的自然数”，这句话用不了二十个单词
就可以表达。然则，这个数字也是可以用少于20个单词就可以表达的自然数。这就自相矛
盾了。
...
伪，但不能被穷举。这最多就是我们的目的，但没有人说目的就总可以实现啊。为了限制
这一点，数学家们又定义了很多的定义上的约束的方法，但本质上就是说，你必须用直接
的可以直接的判断标准（elementary predicate）定义谓词，不能用这之外的东西来定义
它。
...
.. warning::
   ...
   果你说的是“不收这个中断”，可以范围就太大了，这个打算没法实施。这样的设计，只
   能用作很高层的参考，设计的控制程度是不足的。
...
量词在谓词逻辑中是个很不好处理的东西，因为它没法直接参与集合运算，所有通常需要
很多特殊的手法来处理。在Alloy这种建模语言中，一种很常用的算法是Skolem提出的，所
以叫Skolemization。可以在一定程度上把推理空间变得更接近集合。所以，我们需要知道
一下它的基本概念。

如果一个谓词中提到一个变量，而我们没有说它对于某个集合有效还是对于某个集合的部
分成员有效，我们的约束对这个变量就没有范围要求（相当于可以取值所有对象的值），
这个变量就叫自由变量，这种变量不会对我们的推演空间有任何约束，它是Free的。否则，
它们就是Bound的。
...
y都有x），自反（Reflexive，每个成员至少和自己关联等等），传递（Transitive，如果
a和b有关系，b和c有关系，则a和c必然有关系），有序（Ordered，Total Ordered，
Partial Ordered），对称（Symmetric，Anti-symmetric），矩阵可逆（invertible），
等价（Equivalence）等等，为此也有很多的定理，单独是研究这种函数的问题。
...
.. warning::
   ...
   题，这个成本就可能无限增大，最终问题就不可解决了。

我们这里主要点了一些关键的概念，以便读者在后面看Alloy相关的东西，其他细节的定义，
公理，定理，推论，还是看书吧。
...
是定义一个全集空间，然后用集合语言进行范围搜索，然后和一些意图进行匹配，看你“设
计”定义的范围，是不是越过“意图”的范围，从而判断这个逻辑设计是否越过了范围。
...
类别。按我们一开始的例子，这个世界的对象的全集（Alloy中用univ常数表示）可能是
{Apple, Orange, Pear, Red, NotRed}，到了实际推理的时候，你制定你每种sig要多少个
Alloy在那个范围里面给你推理就是了。
...
.. warning::
   ...
   CPU是否接受这个中断。但中断必须发个一个CPU，这个信息，在整个“世界”中，总是存
...
sig也看作是关联，一元关联（Unary）。这其实都是Atom的组合。
...
2. 我们头脑对世界的全部认识其实本质也是这样一个模型（只是更大，而且很多时候没有
   进行完整的穷举），Can be said clearly的东西也只是Can be said而已，不代表事实。
   但我们用这种方法弄清楚我们的大脑在进行决策的时候，是根据什么认知的判断来得到
   结果的。
...
总的来说，Alloy的模型是让你同股sig定义一个世界，用fact限定这个世界的关系（例子
中只有二元关系，但Alloy可以支持多元关系，只是二元关系通常用得最多），然后你通过
Assert确认你的限定条件之下，某些目标是否就可以成立了。或者通过run pred确认一下
你的目标在这个限定之下是不是有可能成立的。
...
这完全看你要怎么建模了，它还可以是这样的（这是原书的一个地址本的例子）：::
...
程，无论遇到什么问题，只要完成这个特定的流程，你就能得到一个一致的结果。
...
.. warning::
   ...
   行，不表示它未来修改和升级以后还能运行，忽略的架构设计，没有为未来留余地，设
   计是简单了，但也很快就没有了未来了。
...
::
  ...
  注：RISCV的这个模型包括两个定义：其中riscv.als定义RVWMO（弱内存序），
      ztso.als定义的是强内存序，后者只是前者的一点补充，我们这里只看前者。
...
.. note::
   ...
   这也是自然哲学论中所说的，如果两个对象（概念）的属性全部相同，那么这两个对象
   就是同一个对象。
...
pred定义的是一个还没有使用的范围（还记得它本身在集合论中的原意吧），所以pred本身
不对系统造成约束，但它可以用于run，也可以用于fact的定义。
...
1. ^用于计算一个关联集合的可迁移闭包（这同样是集合论概念），这种方法很容易定义
   一个全序关系。如果你定义了一个e1-e2-e3-e4这样的序列，关联了e1和e2，e2和e3。^
   可以直接求出所有成员之间的先后关系，自动帮你补上e1和e3的关系。如果用iden（这
   是Alloy的常量，表示univ x univ）和它做一个交集，如果交集是
   :math:`emptyset`
   ，就说明没有任何循环了。

2. 判断一对关系是否在一个连续的群中，只要判断它是否在rel+~rel里面就可以了。

3. disj是集合论中的disjoin关键字，表示两者正交。
...
最基本的承诺，当然是ppo就是^gmo的子集了（这个定义其实很有意思，我自己反正这么复
杂的，所有的ppo如何组合到gmo中的描述，最终总结出来居然是这么简单的数学定义）。

其他的都是人为承诺，我们在指令上承诺的规矩，也是我们建模的核心。
