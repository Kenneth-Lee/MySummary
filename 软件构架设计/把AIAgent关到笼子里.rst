...
要求你每次创建新的实例，就需要独立指配用户名。每个容器的0，都可以是同一个本地
用户，甚至就是当前的宿主用户。

LXC不是虚拟机，而是一个NS（名称空间，namesapce），所以其实它不需要有虚拟机的概
念，只要给定一个config，里面描述隔离要求，就可以完成隔离，你甚至不需要指定
...
坏没有共享给它的文件。缺点是这会导致lxc-attach set_group()调用失败（只是一个提
示，不影响使用，可以通过lxc-attach -e参数消除），我觉得这说明LXC发展不成熟，但
还是那句话，核心功能还是稳定的。
...
机暴露给虚拟机的adb，这经常会断），很省事，唯一问题就是慢，而慢就是一切，这一
点我在总结中讨论。
