...
用户运行不了，也容易造成破坏，我这里重点考虑unprevileged模式。

和用户隔离相比，容器隔离最大的好处就在它的用户管理。每个容器都是同一组用户来管
理的，不需要像用户隔离那样每个实例都需要产生一个新的用户。

LXC依靠名称空间映射来实现用户对应。每个用户被配置一组sub uid/gid。这些sub
uid/gid的地位可以用于当前用户的名称空间，但地位和一般的uid/guid地位相同。
...
第一个命令创建了一个名称空间，然后在里面运行id，在名称空间中，id命令的运行结果
就是root（0）。

第二个也是一样的动作，但这次创建了一个文件，touch命令还是认为自己是root，但
root（0）被-m参数映射到100000上了，所以你在容器中看到它的用户是0，在host中看，
它就是100000了。但100000这个用户，并不能操作仅kenneth（1000）才有权限的文件，
同样，kenneth也不能操作仅100000有权访问的文件。

这样，每个用户会有65536个子用户可以分割权限，每个容器都是这些子用户，这就不会
要求你每次创建新的实例，就需要独立指配用户名。每个容器的0，都可以是当前用户
kenneth。

LXC不是虚拟机，而是一个NS（名称空间，namesapce），所以其实它不需要有虚拟机的概
念，只要给定一个config，里面描述隔离要求，就可以完成隔离，你甚至不需要制定
rootfs，直接复用本地的rootfs都是可以的。但我们用LXC的目的就是不想它看我们整个
rootfs，所以通常还是会创建一个rootfs，但和虚拟机不同，这些rootfs并不需要“启动”，
我们是在隔离空间中运行一个命令，如果命令需要访问文件，就去rootfs中找而已，并没
有一个启动rootfs的过程。
...
给其他用户。而且为了让容器能看见这个目录，我的home也必须让other可见，这反而增
加安全风险。

一种思路是改用ACL用户权限，需要文件系统支持，不通用。我选择是是另一个简单一点
的方案：把容器的0映射到本地用户上。
...
这样，如果你只用root，甚至都不用lxc-usernsexec管理这个根目录了，容器照样不能破
坏没有共享给它的文件。缺点是这会导致lxc-attach set_group()调用失败，我觉得这也
算是一种没有考虑到的bug，可以通过lxc-attach -e参数消除。只是这说明lxc确实测试
不那么充分，还没有发展成熟。
