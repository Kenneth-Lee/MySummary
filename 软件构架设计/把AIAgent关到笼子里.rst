...
作为名称空间，LXC也使用自己的mount名称空间，宿主的mount不影响容器的mount，如
...
户是排斥的，比如Debian中不让你用root为venv环境做pip安装，所以我一般不会直接用
在容器中用root。我觉得未来这种应用生态应该会改变。
...
Overlay这种用法很灵活，我个人是很喜欢的，但要非常注意危险性。比如你创建了一个
容器A，然后用Overlay创建了另一个B，然后再基于B Overlay了一个C，这时你如果继续
用B，那C就会一起受影响了。所以必须遵守这个基本原则：如果你决定Overlay另一个容
器，那这个容器链条上的所有容器就要决定好不要再用了。

其实不但是COW，容器这种灵活的bind mount能力，很容易造成破坏的。我试过把宿主的
Android Sdk bind mount给了容器，我记错这是一个软链接，直接做了一个rm -Rf
~/Android，这件事本身做得不地道，因为如果真是个软链接，简单rm就够了，当时是
晕了头了，觉得在容器内，无所谓。无论如何吧，反正这样我就在容器中把我的宿主Sdk
删掉了。所以，我觉得bind mount一定要有规律，而且能用ro mount就不要用rw，否则很
容易当作VM，一不小心就会丢失数据。
...
* 它的独立性比容器和用户隔离都要好，用户ID是自己的，屏幕是自己的，文件系统也是
  自己的，本地的目录只能共享过去，没有bind mount一说。这增强了安全性，但不那么
  方便，因为共享目录共享的是宿主的inode，如果你想重新指向，就只有重启虚拟机了。
  而容器是可以随时在宿主上面再bind mount一个文件到容器的rootfs中的。
...
这个验证我其实还没有做完，但因为现在够用了，我就先做到这里了。现在我基本上做主
业（一个Redis集群相关的开发）和一些零散的其他小开发，我就持续放在两个基础容器
中，如果还要做验证的，我就从这两个基本系统上overlay一个新的出来，做完就删掉。

这个使用过程就好像我和另一个人共享了一个或者几个目录一起维护一个工程，它会有部
分权限，可以做一些有限的工作，而我可以看到它的整个工作环境，根据需要收集结果到
我的空间中。我觉得这基本是个可控的感觉了。

我其他的感想主要是针对之前讨论的一个问题的：到底Agent OS的需求是什么，我现在觉
得主要是容器，我觉得Agent OS应该集成足够好用的容器技术，随时可以开很多的容器
实例，可以很容易设置限制范围，把本地的资源根据需要映射给容器，这样在未来才能保
证我们大量引入Agent，而不会担心把自己的信息直接暴露到整个网络上。

这个容器方案甚至不需要现在这么灵活，不需要可以安装任意发行版，直接从本地clone
一个版本进去就可以了，这样设计会简单很多，也不需要image服务，始终都是基于COW能
力新建和合并多个管理实例，提供保存版本和回滚的能力就行了。其他的设施，现有的OS
基础设施基本也够用了。至于使用host资源（比如USB，磁盘，GPU驱动等），这些慢慢打
磨，总是可以搞定的，就不在话下了。
...
当然，现在全部靠我这种喜欢折腾的工程师才能用的接口提供功能肯定不行，但技术方向
定了，剩下的只是人力投入的问题。
